// gold-report-webhook
//
// Server-to-server webhook for the $5 Gold Verification Report paywall
// (mobile/app/(app)/scan/verify-gold.tsx's GoldPaywallCard). This is the ONLY
// thing that ever marks a `gold_report_purchases` row 'paid' — the mobile app
// can insert its own 'pending' row (migration 0013) but can never write
// 'paid' itself (RLS-enforced), matching the same client-owned-parent /
// service-role-only-write split used by high-value-report-webhook.
//
// Once a purchase is confirmed paid, this function triggers the SAME single
// AI call verify-gold-value used to make immediately on submit — that call
...
//
// Same fail-closed convention as high-value-report-webhook: this endpoint
// REJECTS every request until a real payment gateway is selected and
// GOLD_REPORT_GATEWAY_ENABLED=true is set alongside the shared secret.
// Flipping the secret alone does not open the endpoint.
//
...
// identical scheme to high-value-report-webhook/mobile-money-webhook.
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.45.4";
import { corsHeaders } from "../_shared/cors.ts";
import { log, logError } from "../_shared/logger.ts";
import { processVerification } from "../verify-gold-value/index.ts";
...
const GATEWAY_SHARED_SECRET = Deno.env.get("GOLD_REPORT_GATEWAY_SECRET") ?? "";
const GATEWAY_ENABLED = Deno.env.get("GOLD_REPORT_GATEWAY_ENABLED") === "true";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (verificationError || !verification) {
      logError("gold-report-webhook", new Error("Verification not found after payment"), { purchaseId });
    ...
    if (scanError || !scan || !finalResult || !finalResult.bestMatch) {
      logError("gold-report-webhook", new Error("Original scan result not found"), { purchaseId });
      return jsonResponse({ received: true, warning: "scan result not found" });
    }

    // Same hallmark lookup as verify-gold-value/index.ts's handleRequest —
    // duplicated deliberately rather than shared, matching that function's
    // own "zero code dependency" convention for anything outside
    // processVerification itself.
    ...
    if (verificationResponse.status !== 200) {
      ...
      logError("gold-report-webhook", new Error(body?.error ?? "verify-gold-value failed"), {
        purchaseId,
    ...
    log("info", "gold-report-webhook", "purchase paid, verdict generated", {
      purchaseId,
      verificationId: verification.id,
    });
    ...
    logError("gold-report-webhook", err, { purchaseId });
