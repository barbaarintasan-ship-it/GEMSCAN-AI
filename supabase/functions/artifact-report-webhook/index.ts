// artifact-report-webhook
//
// Server-to-server webhook for the $5 Artifact Verification Report paywall
// (mobile/app/(app)/scan/verify-artifact.tsx's ArtifactPaywallCard). This is
// the ONLY thing that ever marks an `artifact_report_purchases` row 'paid' —
// the mobile app can insert its own 'pending' row (migration 0015) but can
// never write 'paid' itself (RLS-enforced).
//
// Once a purchase is confirmed paid, this triggers the SAME single AI call
// verify-artifact-value used to make on submit. Reusing processVerification()
// keeps the "one AI call, never rerun" invariant intact.
//
// Same fail-closed convention as the other report webhooks: REJECTS every
// request until ARTIFACT_REPORT_GATEWAY_ENABLED=true is set alongside the
// shared secret. Flipping the secret alone does not open the endpoint.
//
// Expected payload:
...
import { processVerification } from "../verify-artifact-value/index.ts";
...
const GATEWAY_SHARED_SECRET = Deno.env.get("ARTIFACT_REPORT_GATEWAY_SECRET") ?? "";
const GATEWAY_ENABLED = Deno.env.get("ARTIFACT_REPORT_GATEWAY_ENABLED") === "true";
...
Deno.serve(async (req) => {
  ...
  try {
    // Atomic claim: a single UPDATE ... WHERE status != 'paid' can only ever
    // succeed once for a given row (Postgres serializes concurrent updates to
    // the same row), so exactly one of two concurrent identical deliveries
    // gets a non-empty result back — no double AI call / double verdict row.
    ...
    if (verificationError || !verification) {
      logError("artifact-report-webhook", new Error("Verification not found after payment"), { purchaseId });
    ...
    if (scanError || !scan || !finalResult || !finalResult.bestMatch) {
      logError("artifact-report-webhook", new Error("Original scan result not found"), { purchaseId });
    ...
    if (verificationResponse.status !== 200) {
      ...
      logError("artifact-report-webhook", new Error(body?.error ?? "verify-artifact-value failed"), {
        purchaseId,
    ...
    log("info", "artifact-report-webhook", "purchase paid, verdict generated", {
      purchaseId,
      verificationId: verification.id,
    });
    ...
    logError("artifact-report-webhook", err, { purchaseId });
