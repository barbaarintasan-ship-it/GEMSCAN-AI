-- 0078_review_sample_rpc.sql
--
-- Review Console S2 — the atomic review transaction. Records a geologist's review
-- (draft or binding), applies per-conclusion states (Confirmed/Corrected/Rejected),
-- and on a binding decision advances the sample lifecycle, posts to the discussion
-- timeline, raises a notification event for the collector + oversight roles, and
-- writes the audit trail — never overwriting the AI assessment (§2, §3, §4, §5, §6).
-- Called by the review-sample Edge Function AFTER RBAC (requireReviewer/Verifier).
-- Service-role only; SECURITY DEFINER (needs geo.* + auth.*).
...
begin
  ...
  -- Audit (§6).
  insert into enterprise.audit_log (actor_id, action, entity_type, entity_id, after, context)
  values (p_actor, 'review_' || v_dec, 'sample', v_sample,
...
-- ── VERIFY ── execute to service_role only ─────────────────────────────────
--   select has_function_privilege('service_role','enterprise.review_sample(uuid,jsonb)','execute'); -- t
--   select has_function_privilege('authenticated','enterprise.review_sample(uuid,jsonb)','execute'); -- f

-- ── ROLLBACK ──  drop function if exists enterprise.review_sample(uuid, jsonb);
