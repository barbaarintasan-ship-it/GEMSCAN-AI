...
// The engine hot path (targeting.rank) called featuresNear ~74 times per ranking,
// each a full O(n) scan of every mapFeature; with 24k features (16,870 drainage +
// 5,960 lineaments) that measured ~322 ms on desktop and multiples of that on a
// budget phone, blocking the JS thread on every cell change and on the Start /
// Finish buttons. This makes each lookup O(features near the point).
//
