// PDF export for the $5 Gold Verification Report result — a separate,
// dedicated builder from lib/pdfReport.ts and lib/verificationPdfReport.ts
// because a GoldVerificationVerdict (purity options, evidence score,
// recommended next tests) doesn't fit either module's shape. Deliberately
// duplicates the small helpers/visual language below rather than importing,
// matching how verify-gold-value keeps zero code dependency on the primary
// scan pipeline or the diamond verification pipeline.
//
...
// responsibility as pdfReport.ts / verificationPdfReport.ts.
import * as Print from "expo-print";
import * as Sharing from "expo-sharing";
import * as FileSystem from "expo-file-system";
import type { GoldVerificationVerdict } from "./goldVerification";

type Lang = "en" | "so";

export type GoldVerificationPdfData = {
  scanId?: string | null;
  verificationId: string;
  generatedAt: string; // ISO timestamp — when the verdict was produced
  photoUri?: string | null; // remote (signed) or local file:// URI
  verdict: GoldVerificationVerdict;
  recommendationLabel: string; // already-localized label (verify-gold.tsx's RECOMMENDATION_LABELS)
};
...
// Mirrors verify-gold.tsx's EVIDENCE_SCORE_BANDS thresholds exactly, so the
// PDF never disagrees with what the app already showed the user.
  ...
  try {
    ...
    if (/^https?:\/\//i.test(uri)) {
      const target =
        ...
        `gemscan-gold-pdf-img-${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
export function buildGoldVerificationReportHtml(data: GoldVerificationPdfData, lang: Lang): string {
    ...
    <section class="block">
      ...
      <div class="value-card">
        ...
        <div class="value-note">${t(
          "Not identification confidence — this reflects how strong and thorough the collected evidence is (tests performed, photos provided, and how consistent they are with each other).",
          "Maaha kalsoonida aqoonsiga — waxay muujinaysaa intay u xoog badan tahay oo u dhamaystiran tahay caddaynta la ururiyay (tijaabooyinka la sameeyay, sawirrada la bixiyay, iyo sida ay isu waafaqsan yihiin).",
        )}</div>
      </div>
    </section>`;
  ...
  const primaryIdHtml = `
    <section class="block">
      <div class="section-head"><span class="section-icon">🔎</span><h2>${t("Possible Material", "Waxa uu Yahay (Qiyaas)")}</h2></div>
      <div class="id-card">
  ...
  const purityHtml = v.estimatedPurityOptions.length
    ? `<section class="block">
         <div class="section-head"><span class="section-icon">🏅</span><h2>${t("Estimated Purity Possibilities", "Qiyaasta Saafinimada")}</h2></div>
         <div class="purity-row">${v.estimatedPurityOptions.map((p) => `<span class="purity-chip">${esc(p)}</span>`).join("")}</div>
       </section>`
    : "";
    ...
    ? `<section class="block">
         ...
         <p class="body-text">${esc(
           v.professionalTestingNote ||
             t(
               "This is not a substitute for XRF analysis or a certified assay office.",
               "Tani maaha beddel falanqaynta XRF ama xafiiska qiimeeyaha rasmiga ah.",
             ),
         )}</p>
       </section>`
    : "";
...
<title>LuulScan Lab ${t("Gold Verification Report", "Warbixinta Xaqiijinta Dahabka")}</title>
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .purity-row { display: flex; flex-wrap: wrap; gap: 8px; }
  .purity-chip {
    display: inline-block; background: #FBF1DC; border: 1px solid #E6CF85; color: #7A5C00;
    border-radius: 999px; padding: 6px 16px; font-weight: 800; font-size: 13px;
  }
...
<body>
  <div class="doc">
    <header class="masthead">
      <div class="brand-row">
        ...
        <div class="brand-text">
          ...
          <div class="brand-company">${t("Gold Verification Report", "Warbixinta Xaqiijinta Dahabka")}</div>
        </div>
      </div>
      <div class="id-meta">
    ...
    <div class="disclaimer-block">
      ...
      ${t(
        "This report was generated by the LuulScan Lab Company digital laboratory system, combining your original scan with the additional gold verification questionnaire and photos you provided. Photo-based analysis cannot confirm chemical composition. This report is NOT an official certificate and does not constitute absolute certainty. For high-value buying, selling, or legal/insurance purposes, independent XRF analysis or a certified assay office is required.",
        "Warbixintan waxaa soo saaray nidaamka shaybaarka dhijitaalka ah ee LuulScan Lab Company, isagoo isku darayay baaritaankaagii hore iyo su'aalaha xaqiijinta dahabka iyo sawirrada aad bixisay. Falanqaynta ku salaysan sawirka ma xaqiijin karto koombaha kiimikaad ee shayga. Warbixintani MAAHA shahaado rasmi ah, mana tilmaamayso hubaal buuxa. Iibinta, iibsiga, ama arrimaha sharciga/caymiska ee qiimo sare leh, waxaa loo baahan yahay falanqayn XRF oo madax-banaan ama xafiis qiimeeye rasmi ah.",
      )}
    </div>

    <footer class="doc-footer">
      <div><b>LuulScan Lab Company</b> — ${t("Gem & Precious Metal Identification Laboratory", "Shaybaar Aqoonsi Dhagxaan iyo Macdano Qaali ah")}<br/>gemscan.ai &nbsp;·&nbsp; support@gemscan.ai</div>
...
export async function generateAndShareGoldVerificationPdf(data: GoldVerificationPdfData, lang: Lang): Promise<void> {
  const photoDataUri = await toDataUri(data.photoUri);
  const html = buildGoldVerificationReportHtml({ ...data, photoUri: photoDataUri }, lang);
  const { uri } = await Print.printToFileAsync({ html, base64: false });

  const safeName =
    "LuulScan-Gold-Verification-Report-" +
  ...
  if (await Sharing.isAvailableAsync()) {
    await Sharing.shareAsync(shareUri, {
      ...
      UTI: "com.adobe.pdf",
      dialogTitle: lang === "so" ? "La wadaag warbixinta xaqiijinta dahabka" : "Share your gold verification report",
    });
  }
}
