// The geological camera itself, as a component rather than a screen.
//
// WHY IT IS NOT A ROUTE ANY MORE
//
// It began life as `app/(app)/enterprise/field-camera.tsx`, reached with
// `router.push` and handing its photographs back through an in-memory slot. That
// works from the sample form, which is a screen. It cannot work from the Add
// Waypoint form, which is a `Modal`: a React Native modal renders in a native
// window ABOVE the router's stack, so pushing the camera underneath it shows the
// geologist their own form with a camera they cannot see behind it.
//
// So the camera moved here and the route became a thin wrapper around it. Both
// callers now get the same lens, the same zoom, the same focus lock and the same
// burst — which is the point. A waypoint photograph WAS being taken by the
// system picker: no zoom, no focus lock, no full-resolution capture. The evidence
// the AI is asked to read was the worst image the phone could produce, while a
// camera built for exactly this job sat one screen away.
//
// What a geologist photographs is not what a phone camera is tuned for: a quartz
// vein in flat light, a fresh broken surface held at arm's length, a contact
// between two units that only reads at a distance. So the controls that matter
// are focus, flash and framing, and they are all one thumb-reach from the shutter.
//
// Two things expo-camera does NOT expose are called out rather than faked:
...
import type { CapturedPhoto } from "../lib/captureHandoff";
 ...
 * The zoom stops.
 *
 * `CameraView` takes zoom as 0..1 across whatever range the lens actually has, not
 * as a magnification — so these are labelled as a position on that range, and only
 * the first is called "1x" because it is the one stop whose true magnification is
 * known on every device.
 */
const ZOOM_STOPS = [0, 0.25, 0.5, 0.75, 1] as const;
...
export function FieldCamera({
  ...
  // Stop any burst still running when the camera goes away, so a screen that has
  // been closed cannot keep firing the shutter into a discarded state setter.
  React.useEffect(() => () => { burst.current.stop = true; }, []);

  const done = React.useCallback(() => onDone(shots), [shots, onDone]);
  ...
  if (!permission.granted) {
    return (
      <View style={styles.fillCentre}>
        ...
        <Pressable onPress={onCancel} style={styles.permGhost}>
