...
//
// MOVED from mobile/lib/geo/commodityModel.ts (Solo→Team shared-targeting Phase 1).
// Zero mobile-only dependencies (no h3, no React Native) — this is a pure
// function of a `PackCommodityProfile` row, so the relocation changes nothing
// about what it computes. See mobile/lib/geo/commodityModel.ts for the
// re-export shim that keeps every existing Solo import unchanged.
//
...
// --------------------------------------------
// It is NOT a calibrated model, and it cannot be. Every rule here is read from
// the profile's own geological content and is labelled `expert_rule` or
// `uncalibrated`. Nothing is labelled `measured` unless it came from the fitted
// priors (lithologyPrior.ts / terrainPrior.ts, Solo-only for now — see Phase 1
// implementation notes on why those two stay mobile-only pending a server-side
// pack equivalent).
//
...
import type { EvidenceRole } from "./evidenceRoles.ts";
import type { PackCommodityProfile, PackData } from "../pack/types.ts";
 ...
 *
 * Every field that legitimately carries diagnostic vocabulary — `deposit_models`
 * (what kind of system this is) and `typical_host_rocks` (what it looks like in
 * the ground), not only the indicator/alteration/mineral lists — is scanned,
 * uniformly across all commodity profiles.
 */
...
function relevanceFor(p: PackCommodityProfile, families: DepositFamily[]): Map<EvidenceRole, RoleRelevance> {
  ...
  //
  // METAMORPHIC compounds this: shear-hosted/remobilised systems in
  // metamorphic terranes are localised by BOTH faults and metamorphic grade
  ...
  //
  // INTRUSIVE / SEDIMENTARY. Host lithology is not an equally strong indicator
  // for every deposit style. For an intrusion-related style (porphyry,
  // pegmatite, greisen, carbonatite) the deposit largely IS the intrusion or
  // its immediate aureole, so a matching mapped rock class is a stronger
  // signal than for a broadly-distributed sediment-hosted style, where
  // mineralisation can sit well away from the "typical" host. Neither
  // multiplier is invented — both are read straight off which families the
  // profile's OWN deposit_models actually match — and neither fires without
  // host_rocks content to condition.
